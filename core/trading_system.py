            ...
            for key in redis_client.scan_iter("trend:*"):
                try:
                    ...
                    try:
                        ...
                        trends.append({
                            "symbol": symbol,
                            "trend": value.decode() if isinstance(value, (bytes, bytearray)) else str(value),
                            "updated_at": None
                        })
                except Exception as e:
                    logger.warning(f"⚠️ خطأ أثناء قراءة اتجاه من Redis: {e}")

...
class TradingSystem:
    ...
    def __init__(self):
        ...
        try:
            self.setup_managers()
            self.setup_flask()
            self.setup_trend_routes()   # ✅ NEW: Web trends routes
    ...
    def setup_flask(self):
        ...
        # ✅ FIX: تحديد مسار templates صراحةً لأن Flask يُنشأ من داخل core/
    ...
    # ✅ FIXED (ROOT CAUSE): Web page + API for trends
    def setup_trend_routes(self):
        ...
        def api_trends():
            ...
            # ✅ 1) المصدر الصحيح غالبًا: TradeManager -> RedisManager -> client
            redis_client = None
            try:
                if hasattr(self.trade_manager, "redis") and self.trade_manager.redis:
                    redis_client = getattr(self.trade_manager.redis, "client", None) or getattr(self.trade_manager.redis, "redis", None)
            except Exception:
                redis_client = None

            # ✅ 2) fallback: ConfigManager لو كان عنده redis/redis_client
            if not redis_client:
                redis_client = getattr(self.config_manager, "redis", None) or getattr(self.config_manager, "redis_client", None)
            ...
            try:
                # ✅ قراءه الرموز من SET الصحيح
                symbols = redis_client.smembers("trend:symbols") or set()

                # إذا كانت فاضية لأي سبب، نعمل fallback scan لكن بدون كسر
                if not symbols:
                    for key in redis_client.scan_iter("trend:*"):
                        k = key.decode() if isinstance(key, (bytes, bytearray)) else str(key)
                        if k == "trend:symbols" or k.endswith(":updated_at"):
                            continue
                        sym = k.replace("trend:", "")
                        symbols.add(sym.encode() if isinstance(key, (bytes, bytearray)) else sym)
                ...
                for sym in symbols:
                    ...
                    # ✅ updated_at key (إذا موجود)
                    updated_raw = redis_client.get(f"trend:{symbol}:updated_at")
                    updated_at_sa = "—"
                    if updated_raw:
                        try:
                            dt = datetime.fromisoformat(updated_raw.decode() if isinstance(updated_raw, (bytes, bytearray)) else str(updated_raw))
                ...
                logger.error(f"❌ Redis trend read error: {e}", exc_info=True)

#!/usr/bin/env python3
"""
نظام معالجة إشارات التداول - النسخة المختصرة
يحافظ على الوظائف الأساسية مع تقليل التعقيد
"""
...
class TradingSystem:
    ...
    def setup_config(self):
        ...
        self.config = {
            ...
            'REQUIRED_CONFIRMATIONS': config('REQUIRED_CONFIRMATIONS', default=2, cast=int),
            'CONFIRMATION_TIMEOUT': config('CONFIRMATION_TIMEOUT', default=1200, cast=int),
            'MAX_OPEN_TRADES': config('MAX_OPEN_TRADES', default=5, cast=int),
            
            # 🔔 إعدادات التحكم في الرسائل
    ...
    def _load_signal_list(self, key):
        """تحميل قائمة الإشارات من .env"""
        signal_str = config(key, default='')
        return [s.strip() for s in signal_str.split(',') if s.strip()]
    ...
    def setup_routes(self):
        ...
        def home():
            return jsonify({
                "status": "active",
                "service": "Trading Signal Processor - Compact",
                "version": "4.0.0",
                "port": self.port
            })
    ...
    def handle_trend_signal(self, signal_data):
        ...
        if trend_changed or self.last_trend_notification != new_trend:
    ...
    def open_confirmed_trade(self, signal_key, signal_category):
        ...
        secondary_signals = pending_data['signals_data'][1:]
        ...
        trade_info = {
            ...
            'secondary_signals': [s['signal_type'] for s in secondary_signals]
        }
    ...
    def handle_exit_signal(self, signal_data):
        ...
        # إرسال إشعار الخروج
        if self.should_send_message('exit', trade):
    ...
    def should_send_message(self, message_type, signal_data=None):
        ...
        type_controls = {
            ...
            'confirmation': True,  # دائماً نرسل تأكيد الاتجاه
            'general': True        # دائماً نرسل الإشارات العامة
        }
...
🕐 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
    ...
    def format_entry_message(self, trade_info, confirmation_data):
        ...
        confirm_count = confirmation_data['unique_signals']
        secondary_count = len(trade_info['secondary_signals'])
        
        secondary_listing = ""
        for i, signal in enumerate(trade_info['secondary_signals'][:3], 1):
...
┃ 📋 الإشارة الرئيسية: {trade_info['signal_type']} (تم التأكيد بـ {len(confirm_count)} إشارات)
┃ 🔔 الإشارات المساعدة: {secondary_count} إشارة
{secondary_listing}┃ 📊 الصفقات المفتوحة: {len(self.active_trades)} من {self.config['MAX_OPEN_TRADES']}
┗━━━━━━━━━━━━━━━━━━━━
